...
test('arrival today remains valid after UTC midnight in Colombia', (t) => {
  ...
  const context = readStayContext(new URLSearchParams('cabin_id=9&check_in=2026-09-08&check_out=2026-09-10&guests=50'));
  assert.deepEqual(context, { cabin_id: '9', guests: '50', check_in: '2026-09-08', check_out: '2026-09-10' });
  const link = new URL(stayHref('/contacto', context), 'https://example.test');
  assert.equal(link.searchParams.get('cabin_id'), '9');
  assert.equal(link.searchParams.get('guests'), '50');
...
test('invalid calendar dates, past stays and out-of-range guests are discarded', (t) => {
  ...
  for (const guests of ['0', '51', '2.5', '-1', 'abc']) {
